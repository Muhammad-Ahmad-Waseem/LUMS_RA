...
def draw_bboxes(image, bboxes, line_thickness):
    line_thickness = line_thickness or round(
        0.0003 * (image.shape[0] + image.shape[1]) * 0.5) + 1
...
def search_label(center_x, center_y, bboxes_xyxy, max_dist_threshold):
    """
    在 yolov5 的 bbox Search for the closest center point in label
    :param center_x:
    :param center_y:
    :param bboxes_xyxy:
    ...
    for x1, y1, x2, y2, lbl, conf in bboxes_xyxy:
        ...
        # Horizontal and vertical distances are less than max_dist
        min_x = abs(center_x2 - center_x)
        min_y = abs(center_y2 - center_y)

        if min_x < max_dist_threshold and min_y < max_dist_threshold:

            # Distance threshold to judge whether it is within the allowable error range
            # pick x, y Average distance in direction
            ...
            if min_dist == -1.0:
                ...
                # If it is not the first time, the smaller distance is preferred
