...
out_file = r'D:\LUMS_RA\Data\Seg_data\DHA_Ph12\tiles.csv'
...
for x_tile_idx in range(x_tiles_num):
    x1 = tile_size * x_tile_idx + tile_size//2
    x2 = x1 + tile_size
    for y_tile_idx in range(y_tiles_num):
        y1 = tile_size * y_tile_idx + tile_size//2
        y2 = y1 + tile_size
        ...
        fa = open(out_file, 'a')
        fa.write("%s,%d,%s,\"POLYGON ((%s))\"\n" % ("DHA", idx, '{}'.format(idx), polygon_str))
        fa.close()
