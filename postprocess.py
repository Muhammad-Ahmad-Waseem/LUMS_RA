...
'''
def assign_geocoords(contours,imgs_dir,npy_file):
...
def get_building_polygon(temporal_collapse, Beta_h, Beta_l, distance, min_area, polygon_buffer, conn=2, watershed_line=True, plot_locs=False, imgs=None,
    ...
    plt.imsave('D:\\LUMS_RA\\Python Codes\\PostProcess\\SN7_methods\\test\\Input_Image.png',temporal_collapse)
...
'''
def process(npy_list, thres1=0.5, thres2_h1=0.9999, thres2_l1=0.7, thres2_h2=0.6, thres2_l2=0.35,
    ...
    #return
    npys = np.array(npys,dtype=np.float16)
    img_num = npys.shape[0]
    
    contours, polygons = get_building_polygon(npy_mean, thres2_h1, thres2_l1, distance=distance, min_area=min_area,
